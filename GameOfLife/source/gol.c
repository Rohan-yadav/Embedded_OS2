...
int main(void)
{
	...
	while (1)
	{
		reproduction_flag = true; //when it is true new generation will happen
		usleep(config_TL / 2); // allow new generation to check in
		reproduction_flag = false; //when it is false it will not allow new generation to generate
		usleep(config_TL / 2); // put a hold on reproduction to update display
		if (++ctr == config_TDISP)
		{
			ctr = 0;
			updateDisplay();  //load new pattern
		}
